...
ONNX_MODEL = './onnx_models/helmet.onnx'
RKNN_MODEL = 'helmet.rknn'
IMG_PATH = './onnx_models/1.jpg'
DATASET = './onnx_models/dataset.txt'
...
# CLASSES = ("excavator", "kc", "njc", "wjjdzj", "cc", "td",
#            "dcyb", "snbcyb", "dc_close", "snbc_close")
# CLASSES = ("fuse", "dropfuse")
# CLASSES = ("smoke", "fire")
CLASSES = ("head", "helmet")
...
def letterbox(im, new_shape=(416, 416), color=(0, 0, 0)):
...
if __name__ == '__main__':
    ...
    # Load ONNX model
    print('--> Loading model')
    ret = rknn.load_onnx(model=ONNX_MODEL, outputs=['342', '401', '460']) # 官方coco80类
    ...
    # ret = rknn.init_runtime('rk1808', device_id='1808') # 这里使用仿真棒才可以
    if ret != 0:
    ...
    '''
        # 注释与不注释这里的RGB到BGR并不影响显示结果和精度
        #   不注释显示的BRG颜色
        #   注释显示的是RGB颜色
        对于shape函数一般表示输出数组的形状，对于二维数组就是输出行与列，对于三维数组，
        shape[0]表示三维数组中包含多少个二维数组，后两个数字表示二维数组的形状
    http://www.uml.org.cn/python/201811131.asp
