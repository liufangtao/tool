...
def gene_anno(filename,boxes,labels,width,height,exportdir):
    ...
    exportdir = os.path.join(exportdir,"xmls")
...
def base_crop(filename_list,images_dir,labels_dir,dst_dir,expand_method,expand_rate):
    ...
    for idx,filename in enumerate(filename_list):
        ...
        for line in open(label_file):
            ...
            height0, width0, depth0 = image.shape[:3]
            ...
            xmin0, ymin0, xmax0, ymax0 = annoutils.expand_center(center_x,center_y,rect_w,rect_h,expand_method,expand_rate,height0, width0)
            #保存上述xml坐标到xml文件
            ...
            filenameE = xmlBuilder.createElement("filename")#filename标签
            filenameContent = xmlBuilder.createTextNode(filename+".jpg")
            filenameE.appendChild(filenameContent)
            annotation.appendChild(filenameE)
 
            size = xmlBuilder.createElement("size")  # size标签
            width = xmlBuilder.createElement("width")  # size子标签width
            widthContent = xmlBuilder.createTextNode(str(width0))
            width.appendChild(widthContent)
            size.appendChild(width)
            height = xmlBuilder.createElement("height")  # size子标签height
            heightContent = xmlBuilder.createTextNode(str(height0))
            height.appendChild(heightContent)
            size.appendChild(height)
            depth = xmlBuilder.createElement("depth")  # size子标签depth
            depthContent = xmlBuilder.createTextNode(str(depth0))
            ...
            nameContent = xmlBuilder.createTextNode(class_id)
            ...
            xminContent = xmlBuilder.createTextNode(str(xmin0))
            xmin.appendChild(xminContent)
            bndbox.appendChild(xmin)
            ymin = xmlBuilder.createElement("ymin")
            
            yminContent = xmlBuilder.createTextNode(str(ymin0))
            ymin.appendChild(yminContent)
            bndbox.appendChild(ymin)
            xmax = xmlBuilder.createElement("xmax")
            
            xmaxContent = xmlBuilder.createTextNode(str(xmax0))
            xmax.appendChild(xmaxContent)
            bndbox.appendChild(xmax)
            ymax = xmlBuilder.createElement("ymax")
            
            ymaxContent = xmlBuilder.createTextNode(str(ymax0))
        ...
        f = open(os.path.join(dst_dir, filename+".xml"), 'w')
        xmlBuilder.writexml(f, indent='\t', newl='\n', addindent='\t', encoding='utf-8')
        f.close()
            # crop_img = image.copy()[ymin:ymax,xmin:xmax]

            # segmentname = gene_SegImage_name(filename,str(class_id),ymin,xmin,ymax,xmax)  #corp图名字重写
...
if __name__ == "__main__":
    ...
    dst_dir =os.path.join(base_dir,"corpxml_rect1.5")  #保存地址  
